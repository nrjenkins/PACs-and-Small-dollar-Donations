...
```{r}
## ZIP codes to ZCTA
zip.areas <- read_excel("Data/Raw Data/zip_to_zcta_2016.xlsx") %>%
  select(ZIP, ZCTA_USE)
sum(duplicated(zip.areas[ , "ZIP"]))

## ZIP to Congressional district
cd <- read_delim("Data/Raw Data/natl_zccd_delim.txt", delim = ",", skip = 1) %>%
  mutate(ZCTA = as.character(ZCTA))

## count ZCTAs that span multiple CDs
cd <- 
  cd %>%
  group_by(ZCTA) %>%
  mutate(weight = 1 / n())

sum(duplicated(cd[ , c("State", "Congressional District", "ZCTA")]))

zip.to.cd <- left_join(
  zip.areas,
  cd,
  by = c("ZCTA_USE" = "ZCTA")
) %>% drop_na(`Congressional District`)
sum(duplicated(zip.to.cd[ , "ZIP"]))
...
indiv.contrib.data.raw <- 
  ...
  select(year, fec_trans_id, donor_id, contrib_name, receipt_id, 
         donor_org_name, standard_org, industry_code, contrib_date, contrib_zip, 
         contrib_state) 

## merge individual contributions with ZIP code to CD data
indiv.contrib.data <- left_join(
  indiv.contrib.data.raw,
  zip.to.cd,
  by = c("contrib_zip" = "ZIP")
)

# Load Opensecrets Roster
candidate.roster.raw <- read_delim("Data/Raw Data/Candidate Data/cands18.txt", delim = "|, |", col_names = FALSE)

## rename variables and remove duplicates
candidate.roster <- 
  candidate.roster.raw %>%
  rename(year = X2,
         fec_trans_id = X4,
         receipt_id = X6,
         first_last_party = X8,
         party = X10,
         running_district = X12,
         current_district = X14,
         current_cand = X16,
         cycle_cand = X18,
         cand_type = X20,
         party_result = X22,
         no_pacs = X24) %>%
  select(receipt_id, first_last_party, party, running_district, 
         current_cand, cycle_cand, cand_type, party_result, no_pacs) %>%
  distinct(receipt_id, .keep_all = TRUE)
...
indiv.contrib.data <- 
  ...
  mutate(fec_trans_id = as.character(fec_trans_id),
         ...
         cand_district = paste(contrib_state, `Congressional District`, sep = ""))
...
indiv.contrib.data <- 
  ...
  mutate(indiv_biz = sum(indiv_ag, indiv_comms_electric, indiv_const_pub_wrks,
                         indiv_defense, indiv_engy_res_envrmt, 
                         indiv_fncs_ins_real, indiv_commerce, 
                         indiv_hlth_edu_human, indiv_legal, indiv_manu,
                         indiv_other, indiv_trans)) %>%
  select(receipt_id, indiv_biz, indiv_labor, indiv_ideological, 
         indiv_self_finance, indiv_cand_committee, indiv_jnt_cand_committee,
         indiv_party_committee)
```
...
pac.contrib.data <- 
  ...
  select(receipt_id, dir_business_pacs, in_kind_business_pacs, 
         ...
         ce_pac_ideological, ce_pac_legal)
...
estimation.data <- 
  ...
  select(opensecrets_id, first_name, last_name, district, party, no_corp_pacs,
         ...
         pct_latino, pct_bachelors, tot_voting_age) %>%
