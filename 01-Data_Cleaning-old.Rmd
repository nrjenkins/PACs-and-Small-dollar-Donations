...
member.data <- 
  ...
  dplyr::select(icpsr, opensecrets_id, first_name.x, last_name.x, district, 
                party, 
...
>>>>>>> parent of 6f7fbd8 (calculating in vs out of state contributions):01-Data_Cleaning.Rmd
industry.cats <- 
  industry.cats %>%
...
nominate.data <- 
  nominate.data %>%
  filter(congress == 116) %>%
  select(icpsr, nominate_dim1, nominate_dim2)

member.data <- left_join(member.data, nominate.data, by = "icpsr")
```
...
<<<<<<< HEAD:01-Data_Cleaning-old.Rmd
  ...
  select(district, no_corp_pacs, new_member, tot_pac_money, num_pac_contribs, 
         ...
         pct_latino, pct_bachelors, tot_voting_age, chamber, nominate_dim1,
         nominate_dim2) %>%
=======
